...
public:
	...
	static void load_copy(vector<Col<T>>& vmatin, ifstream& fin)
	{
...
public:
	...
	static void load(vector<T>& vmatout, ifstream& fout)
	{
		arma::uword	n_size;
		fout.read((char*)&n_size, sizeof(vmatout.size()));

		vmatout.resize(n_size);
		for (auto i = vmatout.begin(); i != vmatout.end(); ++i)
		{
